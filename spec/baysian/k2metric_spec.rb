...
describe "K2Metric" do
  ...
  describe ".construct_nodes" do
    ...
    context "when three nodes are given" do
      ...
      specify { expect(nodes[1].parents.length).to eq 2 }
      specify { expect(nodes[1].parents[0]).to eq 1 }
      specify { expect(nodes[1].parents[1]).to eq 2 }

      specify { expect(nodes[2].rank).to eq 0 }
      specify { expect(nodes[2].parents.length).to eq 1 }
      specify { expect(nodes[2].parents[0]).to eq 2 }
      ...
      specify { expect(nodes[4].parents.length).to eq 0 }
    end
  end
